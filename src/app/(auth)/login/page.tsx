...
const OAUTH_ERRORS: Record<string, string> = {
  ...
  no_email:
    "Your account has no public email. Please use email/password login.",
...
export default function LoginPage() {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof Form | "general", string>>
  >({});
  ...
  useEffect(() => {
    ...
    if (err)
      setErrors({
        general: OAUTH_ERRORS[err] ?? "Sign-in failed. Please try again.",
      });
  }, [searchParams]);
            ...
            {[
              {
                ...
                desc: "Manage tasks with speed and precision",
              },
              {
                icon: <Users size={20} />,
                title: "Team Collaboration",
                desc: "Work seamlessly with your entire team",
              },
              {
              ...
              <div
                key={f.title}
                className="flex items-center gap-4 bg-white/10 border border-white/10 rounded-2xl px-5 py-4"
              >
